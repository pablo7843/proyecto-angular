...
const urlnueva = `https://api.artic.edu/api/v1/artworks?page=1`
...
export class ApiServiceService {
  ...
  public getArtWorks(): Observable<IArtwork[]> {
    this.http.get<{ data: IArtwork[] }>(urlnueva).pipe(
  ...
  public getArtworksFromPage(page:any): any{
    this.http.get<{ data: IArtwork[] }>(`${url}?page=${page}`).pipe(
      map(response => response.data)
    ).subscribe((artworks) => {
        this.artworksSubject.next(artworks);
    }
